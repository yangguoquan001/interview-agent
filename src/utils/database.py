...
def list_all_thread_ids(db_path: str):
    ...
    if not os.path.exists(db_path):
        print(f"❌ 找不到数据库文件: {os.path.abspath(db_path)}")
        return []
    ...
    try:
        ...
        # 3. 打印结果
        if unique_threads:
            print(f"✅ 数据库中共有 {len(unique_threads)} 个活动的会话 (thread_id):")
            print("-" * 40)
            for tid in sorted(list(unique_threads)):
                print(f"📍 {tid}")
            print("-" * 40)
        else:
            print("ℹ️ 数据库中尚未记录任何 thread_id。")
    ...
    try:
        ...
        for table in existing_tables:
            ...
            if 'thread_id' in columns:
                ...
                print(f"🗑️ 已清理表: {table}")
                found_any = True

        conn.commit()
        if found_any:
            print(f"✅ 成功：thread_id '{thread_id}' 的数据已彻底清除。")
        else:
            print(f"ℹ️ 数据库中未发现与 '{thread_id}' 相关的记录。")
